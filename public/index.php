...
// ===== Middleware (সঠিক ক্রমে) =====

// ১. CORS Middleware সবার আগে যোগ করতে হবে
...
$app->post('/api/jobs/start', function (Request $r, Response $res) use ($json, $readJson, $pdo) {
    ...
    $pdo->prepare("INSERT INTO jobs(type,model,payload_json,status) VALUES(?,?,?, 'queued')")
        ->execute([ (string) $d['type'], $d['model'] ?? null, json_encode($d['options'] ?? [], JSON_UNESCAPED_UNICODE), ]);
